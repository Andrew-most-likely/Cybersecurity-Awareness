const $ = (e) => document.querySelector(e),
  qs = (e) => Array.from(document.querySelectorAll(e)),
  appState = {
    progress: {},
    quizAnswers: {},
    badges: [],
      ...
      sections: [
        {
          ...
          content:
            "Passwords are the first line of defense for protecting your online accounts and personal data. They prove that you are the rightful owner of your account every time you log in. If someone gains access to your passwords, they can impersonate you, steal your identity, or gain control of your finances and sensitive information.Many people underestimate how valuable their data is to criminals. Hackers can use stolen passwords to access email, bank accounts, or social media. They may also sell your credentials on the dark web to other criminals. Even if you think you have nothing to hide, your personal information can be used in scams, identity theft, and fraud.Common password weaknesses include using short passwords, predictable words, or reusing the same password across multiple accounts. If one site is breached, attackers can use your reused password to break into others. By understanding the importance of strong passwords, you can take real steps to protect your online life.",
        },
        {
          type: "lesson",
  ...
  QUIZZES = [
    {
      ...
      questions: [
        ...
        {
          question:
            "Which is the most secure type of two-factor authentication?",
          options: [
            "SMS codes",
            "Email codes",
            "Authenticator app",
  ...
  ACHIEVEMENTS = [
    ...
    },
    {
      ...
      condition: () =>
        MODULES.filter(
          (e) => (appState.progress[e.id] || 0) >= e.progress.total
        ).length >= Math.floor(MODULES.length / 2),
    },
    {
      id: "all-modules",
      icon: "🏆",
    ...
    },
    {
      ...
      condition: () =>
        QUIZZES.some((e) => {
          const t = e.questions.every((t, o) => {
              const s = `${e.id}-${o}`;
              return appState.quizAnswers[s] === t.correct;
            }),
            o = e.questions.every((t, o) => {
              const s = `${e.id}-${o}`;
              return void 0 !== appState.quizAnswers[s];
            });
          return t && o;
        }),
    },
    {
      ...
      condition: () => {
        ...
        return (
          QUIZZES.forEach((o) => {
            o.questions.forEach((s, a) => {
              e++, void 0 !== appState.quizAnswers[`${o.id}-${a}`] && t++;
            });
          }),
          t === e && e > 0
        );
    ...
    },
    {
      ...
      condition: () => {
        ...
        return (
          QUIZZES.forEach((e) => {
            e.questions.forEach((s, a) => {
              t++, void 0 !== appState.quizAnswers[`${e.id}-${a}`] && o++;
            });
          }),
          e && o === t
        );
  ...
  return (
    {
      ...
      shield:
        '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 3l7 3v5c0 5-3.5 9-7 10-3.5-1-7-5-7-10V6l7-3z" stroke="white" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>',
      phish:
        '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 12c4 0 8-4 12-4s8 4 8 4-4 4-8 4S7 12 3 12z" stroke="white" stroke-width="1.6"/><path d="M14 9l3 3-3 3" stroke="white" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>',
      refresh:
        '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M21 12a9 9 0 11-9-9c2.52 0 4.93 1 6.74 2.74L21 8" stroke="white" stroke-width="1.6" stroke-linecap="round"/><path d="M21 3v5h-5" stroke="white" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>',
...
function applyTheme(e) {
  document.documentElement.setAttribute(
    ...
    (appState.theme = e),
    ($("#themeToggle").textContent = "dark" === e ? " Light" : " Dark");
}
function percent(e, t) {
  return Math.round((e / Math.max(t, 1)) * 100);
}
...
function checkAchievements() {
  ...
  ACHIEVEMENTS.forEach((t) => {
    !appState.badges.includes(t.id) &&
      t.condition() &&
      (appState.badges.push(t.id), e.push(t));
  }),
    e.length > 0 && (showBadgeNotification(e), updateStatsWidget());
}
function showBadgeNotification(e) {
  e.forEach((e, t) => {
    setTimeout(() => {
      ...
      (t.style.cssText =
        ...
        (t.innerHTML = `<strong style="display:block;margin-bottom:4px;">${
          e.icon
        } ${escapeHtml(e.name)}</strong><span class="muted">${escapeHtml(
          e.description
        )}</span>`),
        document.body.appendChild(t),
        setTimeout(() => {
          (t.style.animation = "slideOutRight 0.3s"),
...
function updateStatsWidget() {
  ...
  QUIZZES.forEach((e) => {
    e.questions.forEach((s, a) => {
      o++, appState.quizAnswers[`${e.id}-${a}`] === s.correct && t++;
    });
  }),
    ($(
      "#statsWidget"
    ).innerHTML = `<div class="stat-card"><p class="stat-value">${e}/${MODULES.length}</p><p class="stat-label">Modules Complete</p></div><div class="stat-card"><p class="stat-value">${t}/${o}</p><p class="stat-label">Quiz Correct</p></div><div class="stat-card"><p class="stat-value">${appState.badges.length}/${ACHIEVEMENTS.length}</p><p class="stat-label">Badges Earned</p></div>`);
}
function markModuleCompleted(e) {
  const t = MODULES.find((t) => t.id === e);
  t &&
    ((appState.progress[e] = t.progress.total),
    renderModules(),
    updateStatsWidget(),
    updateProgressView(),
    checkAchievements(),
    checkCertificate(),
    window.firebaseAuth && window.firebaseAuth.saveProgress());
}
function renderModules() {
  const e = $("#modulesGrid");
  (e.innerHTML = ""),
    MODULES.forEach((t) => {
      const o = appState.progress[t.id] || 0,
        s = t.progress.total,
        a = percent(o, s),
        i = o >= s,
      ...
      (r.className = "module"),
        ...
        r.setAttribute("aria-label", `${t.title} module, ${a}% complete`),
        (r.innerHTML = `${
          i ? '<span class="badge" aria-label="Completed">✓</span>' : ""
        }<div class="module-header" style="display:flex;gap:12px;align-items:flex-start;"><div class="icon" aria-hidden="true">${getIcon(
          t.icon
        )}</div><div style="flex:1;"><h3>${escapeHtml(
          t.title
        )}</h3><p>${escapeHtml(
          t.description
        )}</p></div></div><div class="module-footer"><div class="progress-row" style="margin-top:12px;display:flex;align-items:center;gap:8px;"><div class="progress-bar" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${a}" aria-label="Module progress ${a}%"><div class="progress-fill" style="width:${a}%;"></div></div><div class="progress-text">${a}%</div></div><div style="display:flex;gap:8px;margin-top:12px;justify-content:space-between;"><button class="ghost" data-open="${
          t.id
        }" aria-label="View ${
          t.title
        } details">Start Lesson</button><button class="btn" data-complete="${
          t.id
        }" ${i ? "disabled" : ""} aria-label="${
          i ? "Module completed" : "Mark module complete"
        }">${i ? "✓ Completed" : "Mark Complete"}</button></div></div>`),
        e.appendChild(r);
    }),
    qs("[data-open]").forEach((e) =>
      e.addEventListener("click", (e) =>
        openModal(e.currentTarget.dataset.open)
      )
    ),
    qs("[data-complete]").forEach((e) =>
      e.addEventListener("click", (e) => {
...
function openModal(e) {
  ...
  const o = appState.progress[e] || 0,
    s = t.progress.total;
  (modalContent.innerHTML = `<h2 id="modalTitle">${escapeHtml(
    t.title
  )}</h2><p class="muted">${escapeHtml(
    t.description
  )}</p><div style="margin-top:18px;"><h3 style="font-size:1.1rem;margin-bottom:12px;">Course Sections</h3>${
    Array.isArray(t.sections) && t.sections.length
      ? t.sections
          .map(
            (e, t) =>
              `<section style="margin-bottom:16px;padding:14px;border-radius:10px;border:1px solid var(--glass-border);"><h4 style="margin:0 0 8px 0;font-weight:700;font-size:1rem;">${
                t + 1
              }. ${escapeHtml(e.title || "Section")}${
                e.time
                ...
                e.objectives
                  ? '<ul class="muted" style="margin:8px 0;padding-left:20px;">' +
                    e.objectives
                      .map(
                        (e) => `<li style="margin:4px 0;">${escapeHtml(e)}</li>`
                      )
                      .join("") +
                    "</ul>"
                  : ""
                ...
                e.content
                  ? `<div class="lesson-text" style="margin-top:10px;line-height:1.6;color:var(--text-color);">${e.content}</div>`
                  : ""
              }</section>`
          )
          .join("")
      : '<div class="muted">No detailed content available for this module yet.</div>'
  }</div><div style="margin-top:18px;padding:14px;background:rgba(37,99,235,0.05);border-radius:10px;border:1px solid var(--glass-border);"><strong>Progress:</strong> ${o} of ${s} sections completed (${percent(
    o,
    s
  )}%)</div><div style="margin-top:18px;display:flex;gap:10px;justify-content:flex-end;"><button class="ghost" id="modalCloseBtn">Close</button><button class="btn" id="modalAddProgress" ${
    o >= s ? "disabled" : ""
  }>${o >= s ? "✓ Completed" : "Mark as Complete"}</button></div>`),
    modal.setAttribute("aria-hidden", "false"),
    trapFocus(modal),
    $("#modalAddProgress").addEventListener("click", () => {
      markModuleCompleted(e), closeModal();
    }),
...
function onModalKeyDown(e) {
  "Escape" === e.key && closeModal(), "Tab" === e.key && trapTabKey(e, modal);
}
...
function renderQuizzes() {
  const e = $("#quizContainer");
  (e.innerHTML = ""),
    QUIZZES.forEach((t) => {
      const o = document.createElement("div");
      o.classList.add("quiz-card"), (o.style.marginBottom = "24px");
      const s = t.open || !1;
      let a = `<div class="quiz-header" style="display:flex;justify-content:space-between;align-items:center;"><h3>${escapeHtml(
          t.title
        )}</h3><button class="ghost" data-toggle="${t.id}" aria-label="${
          s ? "Collapse quiz" : "Expand quiz"
        }">${s ? "Close" : "Open"}</button></div>`,
        i = "";
      if (s) {
        let e = 0,
          o = 0;
        t.questions.forEach((s, a) => {
          const i = `${t.id}-${a}`;
          void 0 !== appState.quizAnswers[i] &&
            (o++, appState.quizAnswers[i] === s.correct && e++);
        });
        const s = o === t.questions.length,
          a = s ? Math.round((e / t.questions.length) * 100) : 0;
        s &&
          (i += `<div class="quiz-score"><strong>Your Score: ${a}%</strong> (${e}/${t.questions.length} correct)<button class="ghost" data-retake="${t.id}" style="margin-left:12px;">Retake Quiz</button></div>`),
          t.questions.forEach((e, o) => {
            const s = appState.quizAnswers[`${t.id}-${o}`],
              a = void 0 !== s,
              r = s === e.correct;
            i += `<div class="quiz-question"><strong>${o + 1}. ${escapeHtml(
              e.question
            )}</strong><div class="quiz-options">${e.options
              .map((i, r) => {
                let n = "quiz-option";
                return (
                  a &&
                    (r === e.correct
                      ? (n += " correct")
                      : r === s && (n += " incorrect")),
                  `<div class="${n}" data-quiz="${
                    t.id
                  }" data-question="${o}" data-option="${r}" style="${
                    a ? "cursor:default;" : ""
                  }">${escapeHtml(i)}</div>`
                );
              })
              .join("")}</div>${
              a
                ? `<div class="quiz-result ${r ? "correct" : "incorrect"}">${
                    r ? "✓ Correct!" : "✗ Incorrect"
      ...
      (o.innerHTML = a + `<div class="quiz-body">${i}</div>`), e.appendChild(o);
    }),
    qs("[data-toggle]").forEach((e) =>
      e.addEventListener("click", (e) => {
        const t = e.currentTarget.dataset.toggle;
        QUIZZES.forEach((e) => {
          e.id === t ? (e.open = !e.open) : (e.open = !1);
        }),
          renderQuizzes();
      })
    ),
    qs(".quiz-option").forEach((e) => {
      e.addEventListener("click", (e) => {
        const t = e.currentTarget,
          o = t.dataset.quiz,
          s = parseInt(t.dataset.question),
          a = parseInt(t.dataset.option),
          i = `${o}-${s}`;
        void 0 === appState.quizAnswers[i] &&
          ((appState.quizAnswers[i] = a),
          renderQuizzes(),
          updateStatsWidget(),
          checkAchievements(),
          checkCertificate(),
    ...
    qs("[data-retake]").forEach((e) => {
      e.addEventListener("click", (e) => {
        const t = e.currentTarget.dataset.retake,
          o = QUIZZES.find((e) => e.id === t);
        o &&
          (o.questions.forEach((e, o) => {
            delete appState.quizAnswers[`${t}-${o}`];
          }),
          renderQuizzes(),
          updateStatsWidget());
      });
...
function renderGlossary() {
  const e = $("#glossaryContainer"),
    t = GLOSSARY.map(
      (e) =>
        `<dl class="glossary-term"><dt>${escapeHtml(
          e.term
        )}</dt><dd>${escapeHtml(e.definition)}</dd></dl>`
    ).join("");
  e.innerHTML = t;
}
function renderResources() {
  const e = $("#resourcesContainer"),
    t = RESOURCES.map(
      (e) =>
        `<div class="resource-card"><h3 style="margin:0 0 8px 0;font-size:1.1rem;">${escapeHtml(
          e.title
        )}</h3><p class="muted" style="margin:0 0 12px 0;">${escapeHtml(
          e.description
        )}</p><ul class="muted" style="margin:0;padding-left:20px;">${e.links
          .map(
            (e) =>
...
function renderAchievements() {
  const e = $("#achievementsContainer"),
    t = `<div class="badges-grid">${ACHIEVEMENTS.map((e) => {
      const t = appState.badges.includes(e.id);
      return `<div class="badge-item ${t ? "earned" : "locked"}" aria-label="${
        t ? "Earned" : "Locked"
      }: ${e.name}"><span class="badge-icon">${
        e.icon
      }</span><div class="badge-name">${escapeHtml(
        e.name
      )}</div><div class="muted" style="font-size:0.75rem;margin-top:4px;">${escapeHtml(
        e.description
      )}</div></div>`;
    }).join("")}</div>`;
  e.innerHTML = t;
...
function updateProgressView() {
  ...
  let a = 0,
    ...
    r = 0;
  QUIZZES.forEach((e) => {
    e.questions.forEach((t, o) => {
      r++;
      const s = `${e.id}-${o}`;
      void 0 !== appState.quizAnswers[s] &&
        (a++, appState.quizAnswers[s] === t.correct && i++);
    });
  });
  const n = r > 0 ? percent(a, r) : 0,
    c = t === o && a === r;
  (e.innerHTML = `<div style="display:grid;gap:16px;"><div class="resource-card"><h3 style="margin:0 0 12px 0;">Overall Progress</h3><div style="margin-bottom:16px;"><div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span>Modules Completed</span><strong>${t} / ${o}</strong></div><div class="progress-bar" style="height:12px;"><div class="progress-fill" style="width:${s}%;"></div></div></div><div><div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span>Quiz Questions Answered</span><strong>${a} / ${r}</strong></div><div class="progress-bar" style="height:12px;"><div class="progress-fill" style="width:${n}%;"></div></div></div><div style="margin-top:16px;"><div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span>Quiz Accuracy</span><strong>${
    a > 0 ? percent(i, a) : 0
  }%</strong></div></div></div><div class="resource-card"><h3 style="margin:0 0 12px 0;">Module Details</h3>${MODULES.map(
    (e) => {
      const t = appState.progress[e.id] || 0,
        o = e.progress.total,
        s = percent(t, o);
      return `<div style="margin-bottom:12px;"><div style="display:flex;justify-content:space-between;margin-bottom:4px;"><span>${escapeHtml(
        e.title
      )}</span><span class="muted">${t}/${o}</span></div><div class="progress-bar" style="height:8px;"><div class="progress-fill" style="width:${s}%;"></div></div></div>`;
    }
  ).join("")}</div>${c ? '<div id="certificateContainer"></div>' : ""}</div>`),
    c && showCertificate();
}
function checkCertificate() {
  MODULES.filter((e) => (appState.progress[e.id] || 0) >= e.progress.total)
    .length;
  QUIZZES.forEach((e) => {
    e.questions.forEach((t, o) => {
      void 0 !== appState.quizAnswers[`${e.id}-${o}`] && 0;
    });
  });
}
function showCertificate() {
  const e = $("#certificateContainer"),
    t = new Date().toLocaleDateString("en-US", {
      year: "numeric",
      month: "long",
      day: "numeric",
    });
  e.innerHTML = `<div class="certificate"><h2> Certificate of Completion</h2><p class="cert-text">This certifies that</p><p class="cert-name">Cybersecurity Champion</p><p class="cert-text">has successfully completed the</p><p class="cert-text" style="font-weight:700;font-size:1.2rem;">Cybersecurity Awareness Training Program</p><p class="cert-text" style="margin-top:20px;">Completed on ${t}</p><p class="muted" style="margin-top:16px;font-size:0.9rem;">${
    MODULES.length
  } modules completed • ${QUIZZES.reduce(
    (e, t) => e + t.questions.length,
    0
  )} quiz questions answered</p></div>`;
}
function showSection(e) {
  qs(".section-content").forEach((e) => e.classList.remove("active")),
    qs("nav.sidebar a").forEach((e) => e.classList.remove("active"));
  const t = $(`#${e}Section`);
  t && t.classList.add("active");
  const o = qs(`nav.sidebar a[data-section="${e}"]`)[0];
  o && o.classList.add("active"),
    "quizzes" === e && renderQuizzes(),
    "tools" === e && renderTools(),
    "glossary" === e && renderGlossary(),
    "resources" === e && renderResources(),
    "achievements" === e && renderAchievements(),
    "progress" === e && updateProgressView();
}
function scrollToLearn() {
  document.getElementById("app").scrollIntoView({ behavior: "smooth" });
}
function observeInView() {
  const e = document.querySelectorAll(".module");
  ...
  } else e.forEach((e) => e.classList.add("in-view"));
}
function init() {
  renderModules(),
    renderResources(),
  ...
  $("#exportProgress").addEventListener("click", () => {
    const e = {
        ...
        modules: MODULES.map((e) => ({
          id: e.id,
          title: e.title,
          progress: appState.progress[e.id] || 0,
          total: e.progress.total,
          percentComplete: percent(
            appState.progress[e.id] || 0,
            e.progress.total
          ),
        })),
        quizzes: QUIZZES.map((e) => {
          const t = e.questions.map((t, o) => {
            const s = `${e.id}-${o}`;
            return { userAnswer: appState.quizAnswers[s] ?? null };
          });
          let o = 0,
            s = 0;
          return (
            t.forEach((e, t) => {
              null !== e.userAnswer &&
                (s++,
                e.userAnswer ===
                  QUIZZES.find((t) => t.id === e.id).questions[t].correct &&
                  o++);
            }),
            {
              id: e.id,
    ...
    (s.href = o),
      (s.download = `cybersecurity-progress-${
        new Date().toISOString().split("T")[0]
      }.json`),
      s.click(),
      URL.revokeObjectURL(o);
  }),
  qs("nav.sidebar a").forEach((e) => {
    e.addEventListener("click", (e) => {
      e.preventDefault();
...
function pickChar() {
  const e = config.charSet;
  return e.charAt(Math.floor(Math.random() * e.length));
}
(style.textContent =
  "@keyframes slideInRight{from{transform:translateX(400px);opacity:0}to{transform:translateX(0);opacity:1}}@keyframes slideOutRight{from{transform:translateX(0);opacity:1}to{transform:translateX(400px);opacity:0}}"),
...
const footerModalOverlay = document.getElementById("footerModalOverlay"),
  ...
  footerModalText = document.getElementById("footerModalText"),
  footerModalClose = document.getElementById("footerModalClose");
document.querySelectorAll("a[data-modal]").forEach((e) => {
  e.addEventListener("click", (t) => {
    t.preventDefault();
    const o = e.getAttribute("data-modal");
    "tos" === o
      ? ((footerModalTitle.textContent = "Terms of Service"),
        (footerModalText.innerHTML =
      ...
      : "privacy" === o &&
        ((footerModalTitle.textContent = "Privacy Policy"),
        (footerModalText.innerHTML =
      ...
      (footerModalOverlay.style.display = "flex");
  });
}),
  footerModalClose.addEventListener(
    "click",
    () => (footerModalOverlay.style.display = "none")
  ),
  footerModalOverlay.addEventListener("click", (e) => {
    e.target === footerModalOverlay &&
      (footerModalOverlay.style.display = "none");
  });
