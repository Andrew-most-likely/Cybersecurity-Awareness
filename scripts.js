...
const MODULES = [
  ...
  },
  {
    ...
    description:
      "Learn why keeping your software updated keeps you safe.",
...
const RESOURCES = [
  {
    title: "Password Managers",
    description:
      "Industry-leading password managers to secure your accounts",
    links: [
      {
        name: "1Password",
...
const ACHIEVEMENTS = [
  ...
  },
  {
    ...
    condition: () => {
      ...
      return completed >= Math.floor(MODULES.length / 2);
    },
  },
  {
    id: "all-modules",
...
function updateStatsWidget() {
  ...
  $(
    "#statsWidget"
  ).innerHTML = `<div class="stat-card"><p class="stat-value">${completedModules}/${MODULES.length}</p><p class="stat-label">Modules Complete</p></div><div class="stat-card"><p class="stat-value">${quizzesCorrect}/${totalQuestions}</p><p class="stat-label">Quiz Correct</p></div><div class="stat-card"><p class="stat-value">${appState.badges.length}/${ACHIEVEMENTS.length}</p><p class="stat-label">Badges Earned</p></div>`;
}
...
function markModuleCompleted(id) {
  ...
  if (window.firebaseAuth) {
    window.firebaseAuth.saveProgress();
  }
}
...
function openModal(moduleId) {
  ...
  modalContent.innerHTML = `<h2 id="modalTitle">${escapeHtml(
          ...
          .map(
            (s, idx) =>
              `<section style="margin-bottom:16px;padding:14px;border-radius:10px;border:1px solid var(--glass-border);"><h4 style="margin:0 0 8px 0;font-weight:700;font-size:1rem;">${
                ...
                s.content
                  ? `<div class="lesson-text" style="margin-top:10px;line-height:1.6;color:var(--text-color);">${s.content}</div>`
                  : ""
              }</section>`
          )
          .join("")
      : `<div class="muted">No detailed content available for this module yet.</div>`
  }</div><div style="margin-top:18px;padding:14px;background:rgba(37,99,235,0.05);border-radius:10px;border:1px solid var(--glass-border);"><strong>Progress:</strong> ${prog} of ${total} sections completed (${percent(
    prog,
    total
    ...
    prog >= total ? "disabled" : ""
  }>${prog >= total ? "✓ Completed" : "Mark as Complete"}</button></div>`;
...
function renderQuizzes() {
  ...
  QUIZZES.forEach((quiz) => {
    ...
    let headerHTML = `<div class="quiz-header" style="display:flex;justify-content:space-between;align-items:center;"><h3>${escapeHtml(
      quiz.title
    )}</h3><button class="ghost" data-toggle="${quiz.id}" aria-label="${
      isOpen ? "Collapse quiz" : "Expand quiz"
    ...
    if (isOpen) {
      ...
      const score = allAnswered
        ...
        : 0;
      if (allAnswered) {
        bodyHTML += `<div class="quiz-score"><strong>Your Score: ${score}%</strong> (${correct}/${quiz.questions.length} correct)<button class="ghost" data-retake="${quiz.id}" style="margin-left:12px;">Retake Quiz</button></div>`;
      }
      quiz.questions.forEach((q, qIdx) => {
        ...
        bodyHTML += `<div class="quiz-question"><strong>${
          qIdx + 1
        }. ${escapeHtml(
          q.question
        )}</strong><div class="quiz-options">${q.options
          .map((opt, optIdx) => {
    ...
    quizDiv.innerHTML =
      headerHTML + `<div class="quiz-body">${bodyHTML}</div>`;
  ...
  qs(".quiz-option").forEach((opt) => {
    opt.addEventListener("click", (e) => {
      ...
      if (window.firebaseAuth) window.firebaseAuth.saveProgress();
    });
  });
  ...
  const html = GLOSSARY.map(
    (item) =>
      `<dl class="glossary-term"><dt>${escapeHtml(
        item.term
      )}</dt><dd>${escapeHtml(item.definition)}</dd></dl>`
  ).join("");
...
function renderResources() {
  ...
  const html = RESOURCES.map(
    (resource) =>
      ...
      )}</p><ul class="muted" style="margin:0;padding-left:20px;">${resource.links
        .map(
          (link) =>
...
function renderAchievements() {
  ...
  const html = `<div class="badges-grid">${ACHIEVEMENTS.map((ach) => {
...
function updateProgressView() {
  ...
  const allComplete =
    ...
    quizzesCompleted === totalQuestions;
  container.innerHTML = `<div style="display:grid;gap:16px;"><div class="resource-card"><h3 style="margin:0 0 12px 0;">Overall Progress</h3><div style="margin-bottom:16px;"><div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span>Modules Completed</span><strong>${completedModules} / ${totalModules}</strong></div><div class="progress-bar" style="height:12px;"><div class="progress-fill" style="width:${overallPercent}%;"></div></div></div><div><div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span>Quiz Questions Answered</span><strong>${quizzesCompleted} / ${totalQuestions}</strong></div><div class="progress-bar" style="height:12px;"><div class="progress-fill" style="width:${quizPercent}%;"></div></div></div><div style="margin-top:16px;"><div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span>Quiz Accuracy</span><strong>${
    quizzesCompleted > 0 ? percent(quizzesCorrect, quizzesCompleted) : 0
    ...
    (m) => {
      ...
      return `<div style="margin-bottom:12px;"><div style="display:flex;justify-content:space-between;margin-bottom:4px;"><span>${escapeHtml(
        m.title
      )}</span><span class="muted">${prog}/${total}</span></div><div class="progress-bar" style="height:8px;"><div class="progress-fill" style="width:${p}%;"></div></div></div>`;
    }
  ).join("")}</div>${
    allComplete ? `<div id="certificateContainer"></div>` : ""
  }</div>`;
  if (allComplete) {
    showCertificate();
  }
  ...
  const data = {
    ...
    modules: MODULES.map((m) => ({
      ...
      percentComplete: percent(
        appState.progress[m.id] || 0,
        m.progress.total
      ),
    })),
    quizzes: QUIZZES.map((q) => {
      const questions = q.questions.map((quest, idx) => {
        const key = `${q.id}-${idx}`;
      ...
      questions.forEach((q, idx) => {
        if (q.userAnswer !== null) {
          answered++;
          if (q.userAnswer === QUIZZES.find(qq => qq.id === q.id).questions[idx].correct)
            correct++;
        }
      });
...
window.addEventListener('scroll', () => {
  scrollBtn.style.display = (document.documentElement.scrollTop > 200 || document.body.scrollTop > 200) ? 'block' : 'none';
});
...
const footerModalText = document.getElementById('footerModalText');
...
footerModalClose.addEventListener('click', () => footerModalOverlay.style.display = 'none');
footerModalOverlay.addEventListener('click', e => {
  if (e.target === footerModalOverlay) footerModalOverlay.style.display = 'none';
});